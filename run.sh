...
mkdir -p $CC/$MODE
cd $CC/$MODE
cmake ../.. -DCMAKE_BUILD_TYPE=$MODE -DCMAKE_MODULE_PATH=$(pwd)/../../cmaki -DFIRST_ERROR=1 -G"$GENERATOR" -DCMAKE_C_COMPILER="$CC" -DCMAKE_CXX_COMPILER="$CXX" -DNOCACHE=$NOCACHE -DNOCACHE_LOCAL=$NOCACHE_LOCAL
